...
- 2026-07-15: Pinned baseline `4dcf617` was compared with an immutable candidate in fresh isolated threads. The candidate improved partial and consequential user voice while preserving the existing draft/send boundary. A stricter final candidate passed two unchanged Reply replays plus partial, consequential, and explicit-directive cases. Fresh replays from the edited repo package reproduced both unchanged assent and an explicit PR directive. Evidence level 3: isolated execution on synthetic artifacts. Reply uses Sidekick's battle-tested linked-parent destination, exact-draft approval, verbatim-send, and unchanged-copy fallback contract; a separate Reply transport smoke is unnecessary unless those shared mechanics diverge.
