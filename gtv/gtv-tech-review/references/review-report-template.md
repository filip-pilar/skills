# Review Report — Output Template

Use this template to generate the Review Report artifact after completing all review passes.

---

```markdown
# GTV Application Review Report

> **Review Date**: [date]
> **Pathway**: [Exceptional Talent / Exceptional Promise]
> **Documents Reviewed**: [list what was provided]

---

## Changes Since Last Review
[Only include this section if the user indicates this is a revised submission]

**Improvements:**
- [specific improvement acknowledged]

**Still outstanding:**
- [issues that remain from previous feedback]

---

## Overall Assessment

**Rating**: [Strong / Adequate / Needs Work / Not Ready]

**Submission recommendation**: [Submit after edits / Revise before submission / Build evidence first]

[2-3 sentences summarizing the application's overall strength and the most useful next improvements]

**Most important improvements before submission**:
- [highest-leverage improvement]
- [or "Main remaining work is polish/clarity"]

---

## Document-by-Document Review

### Personal Statement
**Rating**: [Strong / Adequate / Needs Work / Weak]

**Strengths:**
- [specific strength with reference to text]

**Issues:**
- [specific issue with actionable suggestion]
- [specific issue]

**AI Detection Flags:**
- [specific sections that sound AI-generated, with explanation]
- [or "None detected"]

---

### CV
**Rating**: [Strong / Adequate / Needs Work / Weak]

**Strengths:**
- [specific strength]

**Issues:**
- [specific issue]

**Date Check**: [All dates verified consistent / Issues found: list them]

---

### Recommendation Letter 1: [Recommender Name]
**Rating**: [Strong / Adequate / Needs Work / Weak]

**Required Elements Check:**
- Typed and dated: [Yes / No]
- Signed: [Yes / No]
- Telephone number: [Present / Missing]
- Email: [Present / Missing]
- Organisation logo and registered address: [Present / Missing / N/A]
- Author's CV or credentials proof: [Present / Missing / Not provided]
- Max 3 pages (excl. credentials): [Within limit / Over]
- Author is established expert in digital tech: [Appears so / Unclear / Concern]
- Author has known applicant's work 12+ months: [Confirmed / Not stated / Concern]
- Evidence fit: [Strong / Adequate / Weak — which evidence can they verify?]
- Recency / independence: [Recent / Older but relevant / Stale / Internal / External]
- Public credibility: [Independently verifiable / Internal authority only / Unclear]
- Stronger recent alternatives visible: [No / Yes — who and why they may be stronger]
- Material weakness if not fixed: [No / Yes — explain if evidence is strong but this recommender cannot verify it]
- Written specifically for this application: [Yes / Appears generic]
- Provides different examples from other letters: [Yes / Overlaps with Letter X]

**Strengths:**
- [specific strength]

**Issues:**
- [specific issue]

---

### Recommendation Letter 2: [Recommender Name]
[Same structure]

---

### Recommendation Letter 3: [Recommender Name]
[Same structure]

---

### Evidence Document 1: [Title]
**Target Criterion**: [which]
**Rating**: [Strong / Adequate / Needs Work / Weak]

**Template Check:**
- Headline: [Clear / Vague / Missing]
- Proof: [Strong / Adequate / Weak / Missing]
- Explanation: [Strong / Adequate / Weak / Missing]
- Call to action: [Present / Missing]

**Strengths:**
- [specific strength]

**Issues:**
- [specific issue]

---

[Continue for all evidence documents]

---

## Cross-Document Consistency

### Date Inconsistencies
| Item | Document A | Document B | Discrepancy |
|---|---|---|---|
| [role/event] | [date in doc A] | [date in doc B] | [what's different] |

[or "No date inconsistencies found"]

### Name/Title Inconsistencies
| Item | Variants Found | Documents |
|---|---|---|
| [company/person/title] | [variant 1, variant 2] | [which docs] |

[or "No naming inconsistencies found"]

### Metric Inconsistencies
| Metric | Variants Found | Documents |
|---|---|---|
| [metric] | [value 1, value 2] | [which docs] |

[or "No metric inconsistencies found"]

### LinkedIn Alignment Notes
- [any alignment issues to check against LinkedIn]

---

## Adversarial Tech Nation Simulation

*These are the questions a skeptical assessor would ask after reading your application:*

1. [Tough question about a specific claim or evidence gap]
2. [Tough question]
3. [Tough question]
4. [Tough question]
5. [Tough question]

**Most Vulnerable Points:**
- [The weakest aspect of the application, with explanation]
- [Second weakest]

---

## Positioning Assessment

**Narrative Distinctiveness**: [Memorable / Adequate / Generic]
**What an assessor would remember**: [One sentence — what stands out about this application after reading 50 others?]
**Positioning Suggestions**: [Specific suggestions for making the application more distinctive and memorable — what story should the applicant be telling?]

---

## AI Detection Summary

**Overall AI Risk**: [Low / Moderate / High]

**Flagged Sections:**
| Document | Section/Passage | Pattern Detected | Risk Level |
|---|---|---|---|
| [doc name] | "[quoted text or description]" | [which AI pattern] | [Low/Med/High] |

**Recommendations:**
- [specific rewriting suggestions]

---

## Priority Fixes

Ranked from most to least critical:

1. **[CRITICAL]** [Description of the most important fix needed]
2. **[CRITICAL]** [Second most important]
3. **[HIGH]** [Important fix]
4. **[MEDIUM]** [Moderate priority]
5. **[LOW]** [Nice to have]
[continue as needed]

**Evidence that doesn't yet exist**: If any fix above requires evidence the applicant hasn't created yet, note it separately:
- [e.g., "OC3 needs a second evidence document — consider speaking at a conference or publishing; timeline: 2-3 months"]
- [or "All fixes can be addressed with existing materials"]

---

## MVE Document Comparison
[Only include if user provided their MVE Document]

**Planned items that are missing from written documents:**
- [item from MVE that didn't make it into any document]

**Items that lost specificity:**
- [item where written version is vaguer than the bullet points]

---

## What Happens After Submission
- **Endorsement decision**: 5-8 weeks (3 weeks for AI/cybersecurity)
- **If endorsed**: apply for visa within 3 months
- **If refused**: you can reapply once with additional evidence — the refusal letter will state specific reasons, address each one directly
- **Simultaneous application**: if endorsement refused, visa fee refunded minus £25

*This is guidance, not a guarantee of outcome.*
*To iterate: revise your documents based on this feedback and paste them back for another review.*
```
