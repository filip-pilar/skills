...
> **Directory note:** `plans/` throughout this file means *the plans directory
> chosen in SKILL.md Phase 4* — normally `plans/`, but `advisor-plans/` when the
> repo already uses `plans/` for something else. Use whichever was chosen
> consistently; don't split files across both.
...
(The `pnpm` commands above are just an example of the *shape*. Use the exact
commands for this repo's stack — verified during recon, not guessed. It might
be `cargo test` / `go test ./...` / `pytest` / `./gradlew test` / `make check`.
An executor that runs the wrong toolchain fails the plan.)
