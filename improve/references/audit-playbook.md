...
**A note on the playbook's examples:** the category checklists below are language-agnostic, but the illustrative snippets lean web/JS (React, `pnpm`, `npm audit`) because that's a common case — they are *examples of the shape of a finding*, not a scope limit. Translate them to the repo's actual stack: the same "query-per-item in a loop" smell is `select_related` in Django, an N+1 in ActiveRecord, or a per-row `db.QueryRow` in Go; "type escape hatches" is `any`/`as` in TS but `interface{}`, `unsafe`, `# type: ignore`, or reflection elsewhere. Audit what the repo is, not what the examples are.

## Default skip-list

Generated, vendored, and build-output directories are not worth auditing — findings there are noise (you can't change vendored code, generated code regenerates). Unless a task specifically targets one, skip:

```
node_modules  dist  build  out  .next  .nuxt  .svelte-kit  .turbo  coverage
vendor  target  .venv  venv  __pycache__  .mypy_cache  .pytest_cache  .gradle
bin  obj  Pods  .terraform  .serverless  .cache  .vercel  .netlify
```

Also skip lockfiles and minified/generated bundles (`*.min.js`, `*.map`, generated `*.pb.go` / `*_pb2.py`, snapshot fixtures) as audit *targets* — though a lockfile is still valid *evidence* for a dependency finding. `scripts/recon.sh` already applies this list; subagent prompts must pass it too (see SKILL.md Phase 2). When something legitimately generated lives outside these paths, note it and skip it.
