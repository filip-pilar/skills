...
**Start by running the recon script** — `bash scripts/recon.sh` (from the skill directory; pass the repo root as `$1` if you're not in it). It gathers the deterministic facts cheaply so you don't spend reasoning tokens re-deriving them: the HEAD SHA to stamp plans against, churn hotspots, the ecosystem and its candidate build/test/lint commands, CI config locations, the intent/design docs present, and the repo shape (generated/vendored dirs already filtered out). Everything it prints is a **lead, not a fact** — it says so itself; verify the candidate commands before any plan depends on them. If the script can't run (no bash, unusual environment), fall back to gathering the same facts by hand as below.

Then, building on the script's output:
...
- Use the git signal from the script (recent commits, churn hotspots) to tell what's actively evolving vs. frozen; dig deeper with `git log` on specific paths where it matters.
...
- the recon facts that scope the search (languages, frameworks, key directories), **plus the Default skip-list from the playbook** — subagents that don't inherit it will burn budget auditing `node_modules`/`dist`/`vendor`; tell them to skip those and anything else recon flagged as generated,
...
**The plans directory.** By default this is `plans/` at the repo root. If `plans/` already exists for some unrelated purpose (not a prior `improve` run), use `advisor-plans/` instead and say so once, up front. **Whichever you pick, use it consistently** — every `plans/...` path in this skill and its reference files (the index `README.md`, `NNN-slug.md` files, drift checks, `execute`/`reconcile` flows) means *the directory you chose*. Don't write some files to one and some to the other. Plans go in:

```
<plans-dir>/          ← plans/ (default) or advisor-plans/ (fallback)
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If the plans directory already exists from a previous run, **reconcile, don't duplicate**: read its `README.md`, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded plans stale in the index.
...
- `branch` → audit only the current working branch's changes: scope = files changed since the merge-base with the default branch plus their direct importers/callers. Light recon, all categories, usually no subagents. **First resolve a trustworthy base ref**, in this order: `origin/<default>` if the repo has that remote-tracking ref *and* it isn't stale (`git fetch --dry-run` shows nothing, or the user is fine with the local copy); else the local `<default>` branch (`main`/`master`, from recon); else, with no usable base, **stop and say so** — offer to compare against an explicit ref the user names or to run a full audit instead. Never silently merge-base against a ref that doesn't exist (it yields the whole history as "changed" and every finding gets mis-tagged). Once the base is chosen: `git diff --name-only $(git merge-base <base> HEAD)..HEAD`. **Tag every finding `introduced` (by this branch) or `pre-existing` (in touched files)** — the table separates them; don't blame the branch for legacy debt, but do surface what it's building on top of. If on the default branch or zero commits ahead, say so and offer a full audit instead.
