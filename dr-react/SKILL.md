...
- Full score scan: `npx react-doctor@latest --json --scope full`
- Changed regression scan: `npx react-doctor@latest --json --scope changed`
- Root-cause help: `npx react-doctor@latest why <file>:<line>`
- Rule help: `npx react-doctor@latest rules explain <rule>`
...
1. Inspect git status first.
   ...
   - If clean and not explicitly told otherwise, create one branch for the run: `react-doctor/score-<target>-<date>`.
2. Establish baseline with the full score scan.
...
3. Choose one coherent diagnostic cluster.
...
4. Before editing, write a short root-cause note:
...
8. Inspect the diff for score integrity.
...
9. Repeat until a stop condition is met.
